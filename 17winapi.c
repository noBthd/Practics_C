...
LRESULT CALLBACK MainWinProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    switch (msg) {
        ...
        case WM_CREATE:
            CreateWindow("Button", TEXT("Sigma1"), WS_CHILD | WS_VISIBLE | BS_TEXT, 50, 50
                          , 80, 30, hwnd, (HMENU)1, NULL, NULL);

            CreateWindow("Button", TEXT("Sigma2"), WS_CHILD | WS_VISIBLE | BS_TEXT, 150, 50
                          , 80, 30, hwnd, (HMENU)2, NULL, NULL);

            CreateWindow("Button", TEXT("Sigma3"), WS_CHILD | WS_VISIBLE | BS_TEXT, 250, 50
                          , 80, 30, hwnd, (HMENU)3, NULL, NULL);
            return 0;
        case WM_COMMAND:
            if (LOWORD(wp) == 1) { 
                MessageBox(hwnd, "Sigma button is pressed (modal)", "Info", MB_OK | MB_ICONINFORMATION);
            } else if (LOWORD(wp) == 2) {
                MessageBox(NULL, "Sigma button is pressed (non-modal)", "Info", MB_OK | MB_ICONINFORMATION);
            } else if (LOWORD(wp) == 3) {
